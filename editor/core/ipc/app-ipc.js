...
r.on("app:explore-project", () => {
    ...
    var t = i.platform;
    if ("ios" === t || "android" === t || "mac" === t || "win32" === t || "android-instant" === t) Editor.NativeUtils.run(i, function (e) {
        if (e) return Editor.failed(e), void 0
    });
    else if ("wechatgame" === t) {
        require("../wechat-game").run(i)
    } else {
        var o = `http://localhost:${Editor.PreviewServer.previewPort}/build`;
...
}), r.on("app:play-on-device", (r, i) => {
    let t = Editor.App._profile.data,
        o = i ? i.platform : t["preview-platform"];
    if ("browser" === o) {
        let r = t["preview-browser"];
        var p = `http://localhost:${Editor.PreviewServer.previewPort}`;
        if ("default" === r) e.shell.openExternal(p);
        else {
            let e = require("child_process").spawn,
                i = [],
                t = "";
            Editor.isDarwin ? (t = "open", i = [p, "-a", r]) : (t = r, i = [p]), e(t, i, {
                detached: !0,
                stdio: "ignore"
            }).unref()
        }
    } else "simulator" === o && Editor.NativeUtils.runSimulator(i)
}), r.on("app:reload-on-device", () => {
    "browser" === Editor.App._profile.data["preview-platform"] && Editor.PreviewServer.browserReload()
}), r.on("app:query-plugin-scripts", (e, r) => {
    Editor.assetdb.queryMetas("db://**/*", "javascript", function (i, t) {
        if (i) return e.reply(i);
        var o;
        o = require("../../share/build-platforms")[r].isNative ? e => e.isPlugin && e.loadPluginInNative : "editor" === r ? e => e.isPlugin && e.loadPluginInWeb && e.loadPluginInEditor : e => e.isPlugin && e.loadPluginInWeb;
        var p = t.filter(o).map(e => Editor.assetdb.uuidToFspath(e.uuid));
        p.sort(), e.reply(null, p)
    })
});