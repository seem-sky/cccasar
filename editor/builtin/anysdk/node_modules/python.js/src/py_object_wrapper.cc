...
void PyObjectWrapper::Call(const FunctionCallbackInfo<Value>& js_args)
{
	...
	if (js_args.Data()->IsUndefined()) {
		...
		v8::String::Utf8Value param1(js_args.This()->ToString());
		std::string functionName = std::string(*param1);
		if (functionName.find("<function init at") != -1 || functionName.find("<compiled_function init at")!=-1 ||functionName.find("<compiled function init at")!= -1) {
			//  encryption before init
			Local<Value> codeValue = NamedGetter(py_object_wrapper->InstanceGetPyObject(), "__code__");
			
			std::string strCode = *String::Utf8Value(codeValue->ToString());

			if (strCode.find("python_for_js.py") != -1) {
				Local<Context> context = js_args.GetIsolate()->GetCurrentContext();
				Local<String> source = String::NewFromUtf8(js_args.GetIsolate(), "new Error().stack", NewStringType::kNormal).ToLocalChecked();
				Local<Script> script = Script::Compile(context, source).ToLocalChecked();
				Local<Value> result = script->Run(context).ToLocalChecked();

				AnyEncrypt::getInstance()->checkEncrypt(strCode, *String::Utf8Value(result));
			}
