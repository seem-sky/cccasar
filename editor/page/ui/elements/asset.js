...
module.exports = Editor.UI.registerElement("ui-asset", {
    ...
    set multiValues(t) {
        t = !(null == t || !1 === t), this._multiValues = t, this._update(), t ? this.setAttribute("multi-values", "") : this.removeAttribute("multi-values")
    },
    ...
    _update() {
        return this._dummy ? (this.$typeName.textContent = this._type, this.$name.textContent = this._name, this._needUpdated = !1, void 0) : this._multiValues ? (this.setAttribute("empty", ""), this.$name.textContent = "Difference", this._needUpdated = !1, void 0) : this.value ? (this.removeAttribute("empty"), this._needUpdated = !0, Editor.assetdb.queryUrlByUuid(this.value, (t, e) => {
            if (!this._needUpdated) return;
            const i = require("fire-url");
            if (!e) return this.setAttribute("missing", ""), this._name = "Missing Reference...", this.$typeName.textContent = this._type, this.$name.textContent = this._name, void 0;
            this._name = i.basenameNoExt(e), this.removeAttribute("missing"), this.$typeName.textContent = this._type, this.$name.textContent = this._name
        }), void 0) : (this._name = "None", this.setAttribute("empty", ""), this.$typeName.textContent = this._type, this.$name.textContent = this._name, this._needUpdated = !1, void 0)
    },
    _onButtonClick(t) {
        if (t === this.$name && Editor.Ipc.sendToAll("assets:hint", this.value), t === this.$browse) {
            let t = this.type;
            "script" === t && (t = "javascript,coffeescript,typescript"), Editor.UI.fire(this, "search-asset"), Editor.Ipc.sendToPanel("assets", "assets:search", `t:${t}`)
        }
        this.readonly || t === this.$close && (this.value = "", setTimeout(() => {
    ...
    _isTypeValid(t) {
        return t === this.type || cc.js.isChildClassOf(Editor.assets[t], Editor.assets[this.type])
    },
    _onDropAreaMove(t) {
    ...
    _onDropAreaEnter(t) {
        ...
        let i = e[0].id;
        this.invalid = !0, this._requestID = Editor.assetdb.queryMetaInfoByUuid(i, (t, e) => {
            if (this._requestID = null, this.highlighted = !0, this._cacheUuid = i, this.invalid = !this._isTypeValid(e.assetType), !this.invalid) return;
    ...
    _onDropAreaAccept(t) {
        t.stopPropagation(), this._requestID && (Editor.Ipc.cancelRequest(this._requestID), this._requestID = null), this.highlighted = !1, this.invalid = !1, this.value = this._cacheUuid, Editor.UI.fire(this, "change", {
            bubbles: !1,
            detail: {
                value: this.value
            }
