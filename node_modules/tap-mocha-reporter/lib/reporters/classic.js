...
function Classic (runner) {
  ...
  runner.on('bailout', function (bailout, suite) {
    ...
    if (bailed)
      return
    bailed = true
    process.stdout.write(Base.color('fail', 'Bail out! ' + bailout))
  ...
  runner.on('suite end', function (suite) {
    ...
    if (fails.length) {
      ...
      process.stdout.write(indent(failMsg, 2))
      process.stdout.write('\n')
    }
    ...
    if (todo.length) {
      ...
      process.stdout.write(indent(todoMsg, 2))
      process.stdout.write('\n')
    }

    if (skipped) {
      var fmt = Base.color('skip', indent('Skipped: %d', 2))
      process.stdout.write(util.format(fmt, skipped))
      process.stdout.write('\n')
      if (skipMsg.length) {
        process.stdout.write(indent(skipMsg.join('\n'), 4))
        process.stdout.write('\n')
      }
      process.stdout.write('\n')
    }
  ...
  runner.on('end', function () {
    ...
    if (grandTotal === grandPass) {
      process.stdout.write(Base.color('checkmark', '\n  ok'))
      process.stdout.write('\n')
    }
  })
}
